...
contract CypherTargetRouter is Ownable {
    ...
    /// @notice Structure to hold initial target data for bulk initialization
    ...
    /// @notice Sets multiple targets in bulk.
    /// @dev Only the contract owner can call this function. Emits TargetSet events for each target.
    /// @param newTargets Array of targets to be set.
    function setTargets(
        InitialTarget[] calldata newTargets
    ) external onlyOwner {
        for (uint256 i = 0; i < newTargets.length; i++) {
            InitialTarget memory target = newTargets[i];
            require(
                bytes(target.program).length > 0,
                "Program must not be empty"
            );
            require(
                bytes(target.provider).length > 0,
                "Provider must not be empty"
            );
            require(bytes(target.chain).length > 0, "Chain must not be empty");
            require(
                bytes(target.target).length > 0,
                "Target must not be empty"
            );
            targets[target.program][target.provider][target.chain] = target
                .target;
            emit TargetSet(
                target.program,
                target.provider,
